(()=>{
'use strict';
const VERSION='11.51.31-no-auto-focus';
...
// Suppress only the click generated by an actual drag/pinch.
// Camera movement is now exclusively user-driven after the first visible initialization.
...
// A hidden mobile panel has no usable bounds on DOMContentLoaded. Initialize it once
// when it first becomes visible; subsequent panel opens never move the camera.
document.addEventListener('xianxia:panel-open',()=>{
  const a=makeCam('asc'),m=makeCam('map');
  if(!a.initialized||!m.initialized)scheduleInitialFocus();
},true);
