...
for(const [area,stage] of [['qingyun',1],['blackwind',1],['blood',1],['foundation_trial',9],['thunder',1],['marsh',4],['marsh',6],['taixu',7],['taixu',8],['taixu',9]]){
  test('saved '+area+' / stage '+stage+' cold start, movement, next run and reload',()=>{
    ...
    if(!saved.zones?.jiedan_trial)assert.deepEqual(restored.zones?.jiedan_trial,{tree:{},runs:0,safe:0,eliteWins:0,bestStone:0,bestHerb:0,bestKills:0},'new Core Formation trial zone migrates in as an empty record');
