...
function transform(src){
  src=once(src,"const PATCH_VERSION='11.36.0';","const PATCH_VERSION='11.37.2';",'version');
...
""
  ...
  src=once(src,"  const eco=rank('eco1'),cfg=beastConfig();\n  const beastTotal=Math.max(1,Math.round(cfg.total*(BAL.ecoTotal[eco]||1)*plan.enemyCount));\n  const beastInitial=Math.min(beastTotal,cfg.sim+(BAL.ecoSim[eco]||0));","  const beastTotal=0,beastInitial=0;",'legacy run counts disabled');
  ...
  src=once(src,"  run.beastTimer-=dt;if(run.beastLeft>0&&run.beastTimer<=0){spawnBeast();run.beastLeft--;const pressure=rank('eco1')>=3&&enemies.some(e=>e.aggressive&&['basic','guard','chaser','attacker','elite'].includes(e.type));run.beastTimer=pressure?3.0+Math.random()*1.5:5.0+Math.random()*2}","  updateEncounterPacks(dt);",'legacy beast timer');
  src=once(src,"  for(const enemy of enemies){","  const strikeSet=attackStrikeSet();\n  for(const enemy of enemies){",'strike set');
  src=once(src,"if(enemy.aggressive)moveToward(enemy,P.x,P.y,enemy.speed*speedMul,dt);else if(home>5)moveToward(enemy,enemy.homeX,enemy.homeY,enemy.speed*.55,dt);","if(enemy.aggressive){if(strikeSet.has(enemy))moveToward(enemy,P.x,P.y,enemy.speed*speedMul,dt);else{const rr=58+(enemy.r||12),tx=P.x+Math.cos(enemy.slotAngle||0)*rr,ty=P.y+Math.sin(enemy.slotAngle||0)*rr;moveToward(enemy,tx,ty,enemy.speed*.82*speedMul,dt)}}else if(home>5)moveToward(enemy,enemy.homeX,enemy.homeY,enemy.speed*.55,dt);",'attack waiting ring');
  src=once(src,"    if(d<P.r+enemy.r+3&&enemy.cd<=0){","    if(strikeSet.has(enemy)&&d<P.r+enemy.r+3&&enemy.cd<=0){",'attack slots');
  src=once(src,"core:'Core Balance v0.1',spatial:'Spatial v0.1'","core:'Core Balance v0.2',spatial:'Spatial v0.1',encounter:'Encounter Density v0.3',incoming:'Multi-target v0.1'",'balance metadata');
  return src;
}
const API={version:PATCH_VERSION,transform};
if(typeof module!=='undefined'&&module.exports){module.exports=API;return}
try{const src=transform(load(BASE));(0,eval)(src+'\n//# sourceURL=balance_core_v11_37_2.transformed.js');const e=document.querySelector('#buildVersion');if(e)e.textContent=`BUILD ${PATCH_VERSION} · BALANCE`;window.__xianxiaEncounterPatch=API}catch(error){console.error(error);const e=document.querySelector('#buildVersion');if(e)e.textContent=`BUILD ${PATCH_VERSION} · ENC ERR`}
})();