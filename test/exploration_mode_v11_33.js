...
const state={
  ...
  camX:350,camY:230,left:0,top:0,scale:1,viewW:W,viewH:H,
...
function buildBadge(text){const el=$('#buildVersion');if(el)el.textContent=text}
...
function patchInkRuntime(){
  ...
  ctx.drawImage=(...args)=>{
    if(state.active&&args.length>=3){
      ...
      if(/\/runtime\/backgrounds\/(qingyun|blackwind|blood|thunder)\.png/.test(src))return;
      if(args.length===9&&/player_core|qingyun_(stone_boar|wind_wolf)|blackwind_(horned_yak|ink_panther)|blood_(armored_bear|ember_fox)|thunder_(stone_rhino|lightning_leopard)/.test(src)){
        const k=.90,dx=args[5],dy=args[6],dw=args[7],dh=args[8],ground=dy+dh;
...
function updateReturnGuide(s,remaining){
  ...
  const target=worldToScreen(EXIT_APPROACH.x,EXIT_APPROACH.y),p=s.P||{x:350,y:230},player=worldToScreen(p.x,p.y);
...
function screenToWorld(e){return{x:clamp((e.clientX-state.left)/state.scale,11,689),y:clamp((e.clientY-state.top)/state.scale,11,449)}}
