...
const AFF_HINT={eco1:'요수 개체 수가 늘고 처치 영석 기대값이 오른다.',eco2:'요수가 더 자주 무리 지어 등장해 광역 법술의 가치가 커진다.',eco3:'희귀·강화 요수 출현과 고보상 전투가 늘어난다.',fate1:'산수와 탐보서가 등장해 전리품 경쟁이 시작된다.',fate2:'산수가 비보를 지닐 가능성과 빼앗을 보상이 커진다.',fate3:'붙잡으면 영초를 남기는 영수가 출현한다.',res1:'고정 영맥이 드러나 영석 채굴 루트가 생긴다.',res2:'영맥에 정예 수호수가 붙지만 채굴 가치도 커진다.',res3:'영맥 규모와 정예 보상이 크게 증가한다. R5 폭주 요수는 처치하지 않아도 영맥 곁에서 채굴이 계속된다.',storm1:'낙뢰 빈도와 연속·복합 패턴이 증가한다. 천뢰 직격 시 뢰흔을 얻는다.',storm2:'단계마다 낙뢰 전조 시간이 길어지고 직격 반경이 커져 일부러 맞기 쉬워진다.',storm3:'천뢰 직격 피해를 단계별로 줄여 연속 직격 수련의 안정성을 높인다.',miasma1:'폭발형 출현과 폭발 위치 판단을 강화한다.',miasma2:'호령형과 폭발형 혼합팩을 활성화한다.',miasma3:'수호막형과 3종 정예 조합을 활성화한다.',formation1:'진안 활성 시 수호령 쇄도가 강해진다. R3부터 추가 수호령, R5에서 후반 웨이브가 강화된다.',formation2:'축기 8층부터 결절이 진법을 방해한다. 파괴하면 해당 방해가 사라지고 수성시간이 1초 줄어든다.',formation3:'축기 9층 태허진령이 태허대진을 전개한다. 살아있는 결절마다 보스 피해감소 15%, 파훼 시 4초 파진 상태가 된다.'};
...
function renderAll(){renderTraining();renderMap();renderAreaPicker();devTools();document.dispatchEvent(new CustomEvent('xianxia:progression-rendered',{detail:{source:'progression-runtime'}}))}function scheduleRender(){if(renderQueued)return;renderQueued=true;requestAnimationFrame(()=>{renderQueued=false;renderAll()})}function observe(){for(const e of[$('#realm'),$('#stone'),$('#herb'),$('#area')])if(e)new MutationObserver(scheduleRender).observe(e,{childList:true,characterData:true,subtree:true})}
addCss();normalizeSkillUnlocks();setupTabs();bindRunBalance();observe();renderAll();requestAnimationFrame(()=>{focusTraining();focusMap()});
window.__xianxiaProgression={version:'11.51.0',trainingPath:TRAIN,render:renderAll,buyStageTraining:buyTrain,breakthroughStage:breakthrough,buyAffinity,unlockArea,focusStageTraining:focusTraining,focusCurrentRealm:focusMap,jumpMilestone:jumpPreset,timingReport,costHtml,effectText,guidance,spellList:SPELLS,BASIC_ID};
})();
