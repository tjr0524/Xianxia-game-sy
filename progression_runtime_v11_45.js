...
taixu:{root:{x:1539,y:644},branches:{eco:[{x:1440,y:731},{x:1386,y:752},{x:1312,y:794}],fate:[{x:1453,y:678},{x:1396,y:668},{x:1342,y:656}],res:[{x:1619,y:670},{x:1659,y:636},{x:1681,y:586}],formation:[{x:1631,y:716},{x:1675,y:743},{x:1727,y:772}]},gate:{jiedan_trial:{x:1539,y:746}}},
jiedan_trial:{root:{x:1539,y:844},branches:{},gate:{}}
};
const AFF_HINT={eco1:'요수 개체 수가 늘고 처치 영석 기대값이 오른다.',eco2:'요수가 더 자주 무리 지어 등장해 광역 법술의 가치가 커진다.',eco3:'희귀·강화 요수 출현과 고보상 전투가 늘어난다.',fate1:'산수와 탐보서가 등장해 전리품 경쟁이 시작된다.',fate2:'산수가 비보를 지닐 가능성과 빼앗을 보상이 커진다.',fate3:'붙잡으면 영초를 남기는 영수가 출현한다.',res1:'고정 영맥이 드러나 영석 채굴 루트가 생긴다.',res2:'영맥에 정예 수호수가 붙지만 채굴 가치도 커진다.',res3:'영맥 규모와 정예 보상이 크게 증가한다. R5 폭주 요수는 처치하지 않아도 영맥 곁에서 채굴이 계속된다.',storm1:'낙뢰 빈도와 연속·복합 패턴이 증가한다. 천뢰 직격 시 뢰흔을 얻는다.',storm2:'단계마다 낙뢰 전조 시간이 길어지고 직격 반경이 커져 일부러 맞기 쉬워진다.',storm3:'천뢰 직격 피해를 단계별로 줄여 연속 직격 수련의 안정성을 높인다.',miasma1:'폭발형 균열석갑충을 출현시킨다. 균열석갑충 처치 시 자운정수 +1.',miasma2:'묵령원과 폭발형 혼합팩을 활성화한다. 묵령원 처치 시 자운정수 +1.',miasma3:'옥린천산갑과 3종 정예 조합을 활성화한다. 옥린천산갑 처치 시 자운정수 +1.',formation1:'진안 활성 시 수호령 쇄도가 강해진다. R3부터 추가 수호령, R5에서 후반 웨이브가 강화된다.',formation2:'축기 8층부터 결절이 진법을 방해한다. 파괴하면 해당 방해가 사라지고 수성시간이 1.5초 줄어든다.',formation3:'축기 9층에서 3결절 태허대진이 열린다. 결절을 깨면 수성시간이 줄어든다. 결단 시련에서는 태허진령이 이 대진의 완성형을 사용한다.'};
...
function jumpJiedanReady(){
  if(snap().phase==='run')return;
  jumpPreset('f9');
  const M=ensure(state()),f9=TRAIN.find(stage=>stage.id==='f9');
  if(f9)for(const n of f9.nodes){
    if(!M.trainingNodes?.[n.id]){
  ...
  M.zones.taixu??={tree:{}};
  M.zones.taixu.tree??={};
  M.zones.taixu.tree.formation1=Math.max(3,+M.zones.taixu.tree.formation1||0);
  M.zones.taixu.tree.formation2=Math.max(3,+M.zones.taixu.tree.formation2||0);
  M.zones.taixu.tree.formation3=Math.max(1,+M.zones.taixu.tree.formation3||0);
  M.unlocked.jiedan_trial=1;
  M.area='jiedan_trial';
  M.stone=0;M.herb=0;M.herb2=0;M.herb3=0;
  M.thunderMark=0;M.purpleEssence=0;M.taixuSigil=0;
  M.formationSkills??={version:1,daoMarks:0,ranks:{},traits:{}};
  M.formationSkills.daoMarks=0;
  M.events={...(M.events||{}),foundationInsight:1,foundationTrialCompleted:1,jiedanTrialCompleted:0};
  debug.replaceState(M);
  document.dispatchEvent(new CustomEvent('xianxia:dev-preset-applied',{detail:{id:'f9_complete_jiedan',realm:{...M.realm},area:M.area,capital:0}}));
  scheduleRender();
}
...
function devTools(){
  ...
  for(const[id,p]of Object.entries(PRESETS)){const b=document.createElement('button');b.textContent=p.label;b.onclick=()=>jumpPreset(id);w.querySelector('.dev-milestone-grid').appendChild(b)}const jb=document.createElement('button');jb.textContent='축기 9층 완성 · 결단 시련';jb.onclick=jumpJiedanReady;w.querySelector('.dev-milestone-grid').appendChild(jb)
